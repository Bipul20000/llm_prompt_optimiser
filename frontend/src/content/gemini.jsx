...
const mountButton = (inputElement) => {
    ...
    const updatePosition = () => {
        const el = document.querySelector('rich-textarea') || document.querySelector('[contenteditable="true"]');
        if (el) {
            wrapper.style.display = 'flex';
    ...
    const handleOptimize = async () => {
        ...
        let editableTarget = el;
        if (el.tagName.toLowerCase() === 'rich-textarea') {
            const innerDiv = el.querySelector('div[contenteditable="true"]');
            if (innerDiv) editableTarget = innerDiv;
        }

        const rawText = (editableTarget.value !== undefined ? editableTarget.value : editableTarget.innerText) || '';
        ...
        try {
            ...
            if (response && response.optimized_prompt) {
                ...
                editableTarget.textContent = response.optimized_prompt;

                editableTarget.dispatchEvent(new InputEvent('input', {
                    bubbles: true,
                    composed: true,
                    inputType: 'insertText',
                    data: response.optimized_prompt
                }));
                editableTarget.dispatchEvent(new Event('change', { bubbles: true, composed: true }));

                if (editableTarget !== el) {
...
const observer = new MutationObserver((mutations) => {
    const inputElement = document.querySelector('rich-textarea') || document.querySelector('[contenteditable="true"]');
...
const initialInput = document.querySelector('rich-textarea') || document.querySelector('[contenteditable="true"]');
if (initialInput) {
